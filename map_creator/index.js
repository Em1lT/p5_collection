...
function getPossibleObjects() {
  return possibleObjects.filter(
    (obj) => obj.type !== "road" && obj.type !== "air",
  );
}
  ...
  for (let i = 0; i < gridSize; i++) {
    for (let j = 0; j < gridSize; j++) {
      if (grid[i][j].type === "air") {
        if (
          grid[i][j].location.x < x &&
          grid[i][j].location.x + gridWidth > x
        ) {
          if (
            grid[i][j].location.y < y &&
            grid[i][j].location.y + gridWidth > y
          ) {
...
function calculateTypes() {
  ...
  for (let i = 0; i < gridSize; i++) {
    for (let j = 0; j < gridSize; j++) {
      types[grid[i][j].type] = types[grid[i][j].type]
        ? types[grid[i][j].type] + 1
        : 1;
    }
  }
  console.log(types);
...
function updatePath() {
  if (path.length < gridSize * gridSize - roads.length) {
    if (isFirstTile) {
      ...
      const adjacentElements = getNextRandomAdjacentElements(
        currentGrid.i,
        currentGrid.j,
      );
      if (adjacentElements.length === 0) {
        for (let i = path.length - 1; i >= 0; i--) {
          ...
          if (elements.length > 0) {
            const adjacentElement =
              elements[Math.floor(Math.random() * adjacentElements.length)];
        ...
        const adjacentElement =
          adjacentElements[Math.floor(Math.random() * adjacentElements.length)];
        currentGrid = adjacentElement;
        path.push(currentGrid);
        const newAdjacentElements = getAdjacentElements(
          currentGrid.i,
          currentGrid.j,
        );
        let compatibleElements = getCompatibleElements(newAdjacentElements);
        // Slider deciedes if there is more water elements, add them to the compatible elements array so the possibility is greater
        if (slider.value() > 75) {
          // more water elements
...
function getCompatibleElements(adjacentElements) {
  const combatibleAdjacentElements = adjacentElements.filter(
    (el) => el.type !== "air",
  );
  const elementCount = {};

  const s1 = combatibleAdjacentElements.reduce((acc, el) => {
    const compatibleElements = getPossibleObjects().find(
      (obj) => obj.type === el.type,
    ).compatibleElements;
    ...
    const notCompatibleElements = possibleObjects.find(
      (obj) => obj.type === el.type,
    ).notCompatibleElements;
...
function getNextRandomAdjacentElements(i, j) {
  const adjacentElements = getAdjacentElements(i, j).filter(
    (el) => el.type === "air",
  );
